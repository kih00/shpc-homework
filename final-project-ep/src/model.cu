...
void SparseMoeBlock::forward(
    ...
    // Bucket tokens per expert (host) - use int for direct memcpy
    ...
    // Copy x_flat to GPUs using P2P (avoid host roundtrip)
    std::vector<Tensor> x_flat_per_gpu(NUM_GPUS);
    for (int dev = 0; dev < NUM_GPUS; dev++) {
        if (!experts_on_device[dev].empty()) {
            if (dev == base_device) {
                // Same device - just reference
                x_flat_per_gpu[dev] = x_flat.view({num_tokens, hidden_size});
            } else {
                // Different device - P2P copy
                x_flat_per_gpu[dev] = Tensor({num_tokens, hidden_size}, dev);
                CHECK_CUDA(cudaMemcpyPeerAsync(
                    x_flat_per_gpu[dev].device_data(), dev,
                    x_flat.device_data(), base_device,
                    num_tokens * hidden_size * sizeof(float), stream));
            }
        }
    }
    CHECK_CUDA(cudaStreamSynchronize(stream));  // Ensure P2P copies complete

    // Expert outputs and token indices per GPU - use int for direct memcpy
    std::vector<std::vector<Tensor>> expert_outputs(NUM_GPUS);
    std::vector<std::vector<std::vector<int>>> expert_token_lists(NUM_GPUS);
    ...
    #pragma omp parallel num_threads(NUM_GPUS)
    {
        ...
        // Pre-allocate buffers once per GPU thread (max size = num_tokens)
        int* h_indices;
        int* d_indices;
        CHECK_CUDA(cudaMallocHost(&h_indices, num_tokens * sizeof(int)));
        CHECK_CUDA(cudaMalloc(&d_indices, num_tokens * sizeof(int)));

        // Pre-allocate expert input buffer on device (max size)
        Tensor expert_in_buf({num_tokens, 1, hidden_size}, dev);
        ...
        for (size_t i = 0; i < experts_on_device[dev].size(); i++) {
            ...
            // Use pre-allocated buffer with view for current expert size
            ...
            // Use pre-allocated buffers + memcpy for faster transfer
    ...
    // Pre-allocate all buffers for scatter (max size = num_tokens)
    int* h_scatter_ids;
    float* h_scatter_weights;
    int* d_scatter_ids;
    float* d_scatter_weights;
    CHECK_CUDA(cudaMallocHost(&h_scatter_ids, num_tokens * sizeof(int)));
    CHECK_CUDA(cudaMallocHost(&h_scatter_weights, num_tokens * sizeof(float)));
    CHECK_CUDA(cudaMalloc(&d_scatter_ids, num_tokens * sizeof(int)));
    CHECK_CUDA(cudaMalloc(&d_scatter_weights, num_tokens * sizeof(float)));
    ...
    for (int dev = 0; dev < NUM_GPUS; dev++) {
        for (size_t i = 0; i < experts_on_device[dev].size(); i++) {
            ...
            // Use pre-allocated buffers + memcpy for faster transfer
            std::memcpy(h_scatter_ids, token_list.data(), tok_count * sizeof(int));
            std::memcpy(h_scatter_weights, weight_list.data(), tok_count * sizeof(float));

            CHECK_CUDA(cudaMemcpyAsync(
                d_scatter_ids, h_scatter_ids,
                tok_count * sizeof(int), cudaMemcpyHostToDevice, stream));
            CHECK_CUDA(cudaMemcpyAsync(
                d_scatter_weights, h_scatter_weights,
            ...
            weighted_scatter_add_kernel<<<grid, block, 0, stream>>>(
                ...
                d_scatter_ids, d_scatter_weights,
                tok_count, hidden_size);

            // Sync before reusing pinned buffers for next expert
            CHECK_CUDA(cudaStreamSynchronize(stream));
        }
    }

    // Free all buffers
    CHECK_CUDA(cudaFree(d_scatter_ids));
    CHECK_CUDA(cudaFree(d_scatter_weights));
    CHECK_CUDA(cudaFreeHost(h_scatter_ids));
    CHECK_CUDA(cudaFreeHost(h_scatter_weights));
