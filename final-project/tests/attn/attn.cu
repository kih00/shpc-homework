...
__global__ void repeat_kv_kernel(
    const float* in, float* out,
    int batch, int kv_heads, int heads, int seq_len, int head_dim, int n_rep) {
...
__global__ void softmax_kernel(
    float* scores, int batch, int heads, int seq_len) {
    int b = blockIdx.z;
    int h = blockIdx.y;
    int i = blockIdx.x;  // query position
...
__global__ void attn_weighted_v_kernel(
...
void attn(float *x, float *cos, float *sin, float *q_proj, float *k_proj, 
    ...
    matmul_transposed_kernel<<<grid_q, block_mm>>>(
        x_gpu, q_proj_gpu, q_proj_out_gpu, bs, hidden_size, q_out_cols);

    dim3 grid_k((kv_out_cols + block_mm.x - 1) / block_mm.x,
                (bs + block_mm.y - 1) / block_mm.y);
    matmul_transposed_kernel<<<grid_k, block_mm>>>(
        x_gpu, k_proj_gpu, k_proj_out_gpu, bs, hidden_size, kv_out_cols);
    matmul_transposed_kernel<<<grid_k, block_mm>>>(
        x_gpu, v_proj_gpu, v_proj_out_gpu, bs, hidden_size, kv_out_cols);
    ...
    rmsnorm_kernel<<<grid_norm_q, BLOCK_NORM>>>(
        q_proj_out_gpu, q_norm_gpu, q_normed_gpu, head_dim, q_vecs);
    rmsnorm_kernel<<<grid_norm_k, BLOCK_NORM>>>(
        k_proj_out_gpu, k_norm_gpu, k_normed_gpu, head_dim, k_vecs);

    // 3) Reshape to (B, H, S, D)
    dim3 grid_reshape((seq_len + BLOCK_RESHAPE - 1) / BLOCK_RESHAPE,
                      num_heads, batch);
    reshape_qkv_kernel<<<grid_reshape, BLOCK_RESHAPE>>>(
        q_normed_gpu, q_transposed_gpu, batch, seq_len, num_heads, head_dim);

    dim3 grid_reshape_k((seq_len + BLOCK_RESHAPE - 1) / BLOCK_RESHAPE,
                        num_kv_heads, batch);
    reshape_qkv_kernel<<<grid_reshape_k, BLOCK_RESHAPE>>>(
        k_normed_gpu, k_transposed_gpu, batch, seq_len, num_kv_heads, head_dim);
    reshape_qkv_kernel<<<grid_reshape_k, BLOCK_RESHAPE>>>(
        v_proj_out_gpu, v_transposed_gpu, batch, seq_len, num_kv_heads, head_dim);

    // 4) RoPE
    dim3 grid_rope_q(seq_len, num_heads, batch);
    rope_kernel<<<grid_rope_q, head_dim / 2>>>(
        q_transposed_gpu, cos_gpu, sin_gpu, batch, num_heads, seq_len, head_dim);
    dim3 grid_rope_k(seq_len, num_kv_heads, batch);
    rope_kernel<<<grid_rope_k, head_dim / 2>>>(
        k_transposed_gpu, cos_gpu, sin_gpu, batch, num_kv_heads, seq_len, head_dim);

    // 5) Repeat KV for GQA
    dim3 grid_rep(seq_len, num_kv_heads, batch);
    repeat_kv_kernel<<<grid_rep, head_dim>>>(
        k_transposed_gpu, k_repeated_gpu, 
        batch, num_kv_heads, num_heads, seq_len, head_dim, n_rep);
    // Reuse attn_out_transposed_gpu as temporary buffer for repeated V
    repeat_kv_kernel<<<grid_rep, head_dim>>>(
        v_transposed_gpu, attn_out_transposed_gpu,
        batch, num_kv_heads, num_heads, seq_len, head_dim, n_rep);

    // 6) Attention scores and softmax
    dim3 grid_scores(seq_len, num_heads, batch);
    attn_scores_kernel<<<grid_scores, seq_len>>>(
        q_transposed_gpu, k_repeated_gpu, attn_scores_gpu,
        batch, num_heads, seq_len, head_dim, scale);
    softmax_kernel<<<grid_scores, 1>>>(
        attn_scores_gpu, batch, num_heads, seq_len);

    // 7) Weighted sum with V
    attn_weighted_v_kernel<<<grid_scores, head_dim>>>(
        attn_scores_gpu, attn_out_transposed_gpu, attn_out_gpu,
        batch, num_heads, seq_len, head_dim);

    // 8) Flatten to (B*S, hidden)
    dim3 grid_flat((hidden_size + BLOCK_FLAT - 1) / BLOCK_FLAT, seq_len, batch);
    flatten_kernel<<<grid_flat, BLOCK_FLAT>>>(
        attn_out_gpu, attn_out_transposed_gpu, batch, num_heads, seq_len, head_dim);

    // 9) Output projection
    int rows = bs;
    dim3 grid_out((hidden_size + block_mm.x - 1) / block_mm.x,
                  (rows + block_mm.y - 1) / block_mm.y);
    matmul_transposed_kernel<<<grid_out, block_mm>>>(
        attn_out_transposed_gpu, o_proj_gpu, output_gpu,
